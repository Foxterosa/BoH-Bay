...
/obj/machinery/gear_painter
	name = "Color Mate"
	desc = "Una maquina para darle a tu ropa un mejor color! Recomendado usar prendas de color blanco para un mejor resultado."
	icon = 'icons/obj/vending.dmi'
...
/obj/machinery/gear_painter/attackby(var/obj/item/I, var/mob/user)
	if(processing.len)
...
/obj/machinery/gear_painter/interact(mob/user as mob)
	if(inoperable())
		return
	user.set_machine(src)
	var/dat = "<TITLE>Color Mate!</TITLE><BR>"
	if(!processing.len)
		dat += "No item ha sido insertado."
	else
		for(var/atom/movable/O in processing)
			dat += "Item inserted: [O]<HR>"
		dat += "<A href='?src=\ref[src];select=1'>Select new color.</A><BR>"
		dat += "Color: <font color='[activecolor]'>&#9899;</font>"
		dat += "<A href='?src=\ref[src];paint=1'>Aplicar un nuevo color.</A><BR><BR>"
		dat += "<A href='?src=\ref[src];clear=1'>Remover color.</A><BR><BR>"
		dat += "<A href='?src=\ref[src];eject=1'>Ejectar item.</A><BR><BR>"

	var/datum/browser/menu = new(user, "colormate","Panel de control", 400, 600, src)
